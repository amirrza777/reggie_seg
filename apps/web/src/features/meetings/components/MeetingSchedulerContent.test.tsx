...
import { MeetingSchedulerContent } from "./MeetingSchedulerContent";
...
describe("MeetingSchedulerContent", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("fetches meetings and shows upcoming by default", async () => {
    render(<MeetingSchedulerContent teamId={10} projectId={1} />);
    await waitFor(() => {
  ...
  it("shows create form when schedule button is clicked", async () => {
    render(<MeetingSchedulerContent teamId={10} projectId={1} />);
    await waitFor(() => screen.getByText("Team Meeting"));
    fireEvent.click(screen.getByRole("button", { name: /schedule team meeting/i }));
