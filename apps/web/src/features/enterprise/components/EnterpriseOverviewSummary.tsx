...
function OverviewMainColumn({ summary, snapshotPrimary, snapshotRoles }: { summary: OverviewSummaryData; snapshotPrimary: MetricGridItem[]; snapshotRoles: MetricGridItem[] }) {
  return (
      ...
      <Card title="Setup checklist" className="enterprise-overview__card">
        <div className="enterprise-overview__checklist">
          <p className="ui-note ui-note--muted">
            {summary.completedChecklistItems}/{summary.setupChecklist.length} setup checks complete
          </p>
      ...
      {summary.status === "loading" ? (
        ...
        <div className="enterprise-overview__action-queue">
          <p className="ui-note ui-note--muted enterprise-overview__action-queue-note">
            {summary.priorityActionCount > 0
              ? `${summary.priorityActionCount} priority actions generated from current setup and growth signals.`
    ...
    <div className="enterprise-overview ui-stack-md">
      <div
        className={`status-alert enterprise-overview__priority-banner ${
          summary.priorityBanner.tone === "error" ? "status-alert--error" : "status-alert--success"
        }`}
      >
