...
export function NewQuestionnaireQuestionList({
  questions,
  preview,
  answers,
  setAnswers,
      ...
      {questions.map((q, i) => (
        <article key={q.uiId} className="questionnaire-editor__question">
          <div className="questionnaire-editor__question-row">
            ...
            {!preview && (
              <Button type="button" size="sm" variant="quiet" onClick={() => setQuestions((qs) => qs.filter((x) => x.uiId !== q.uiId))}>
                Remove
              </Button>
            )}
          </div>
          ...
          {preview ? (
            <>
              <strong className="questionnaire-editor__question-main">{q.label || "Untitled question"}</strong>
              ...
              {q.type === "slider" && (
                <>
                  ...
                  <input
                    ...
                    onChange={(e) => setAnswers((a) => ({ ...a, [q.uiId]: Number(e.target.value) }))}
                  />
            ...
            <>
              ...
              {q.type === "slider" && (
                <FormField
                  ...
                  onChange={(e) =>
                    setQuestions((qs) =>
                      qs.map((x) => (x.uiId === q.uiId ? { ...x, configs: { ...x.configs, helperText: e.target.value } } : x)),
                    )
                  }
                />
              )}
                            ...
                            qs.map((x) =>
                              x.uiId === q.uiId
                                ? {
                                    ...
                                    configs: {
                                      ...x.configs,
                                      options: (x.configs as MultipleChoiceConfigs).options.filter((_: string, i: number) => i !== idx),
                                    },
                                  }
                                : x,
                            ),
