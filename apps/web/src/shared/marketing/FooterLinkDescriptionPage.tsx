...
function FooterLinkDescriptionHero({ category, page }: { category: FooterLinkCategory; page: FooterLinkPageContent }) {
  return (
    <section className="section section--padded footer-link-page">
      <div className="container footer-link-page__hero" data-reveal-group>
        ...
        <div className="hero__cta-row footer-link-page__actions"><Link href={page.primaryCtaHref} className="btn btn--primary">{page.primaryCtaLabel}</Link></div>
      </div>
    </section>
  );
}

function FooterLinkDescriptionDetails({ page }: { page: FooterLinkPageContent }) {
  return (
...
function FooterLinkDescriptionCtaBand({ bottomCta }: { bottomCta: ReturnType<typeof getBottomCtaContent> }) {
  return (
    <section className="section section--gradient footer-link-page__cta-band">
      <div className="container footer-link-page__cta-inner" data-reveal-group>
        <div className="footer-link-page__cta-content" data-reveal><h2 className="display hero__headline footer-link-page__cta-title">{bottomCta.title}</h2><p className="lede footer-link-page__cta-copy">{bottomCta.description}</p></div>
        <div className="hero__cta-row footer-link-page__cta-actions" data-reveal><Link className="btn btn--primary" href={bottomCta.ctaHref}>{bottomCta.ctaLabel}</Link></div>
      </div>
    </section>
  );
}

export function FooterLinkDescriptionPage({ category, page }: { category: FooterLinkCategory; page: FooterLinkPageContent }) {
  const bottomCta = getBottomCtaContent(category, page);
  return <><FooterLinkDescriptionHero category={category} page={page} /><FooterLinkDescriptionDetails page={page} /><FooterLinkDescriptionCtaBand bottomCta={bottomCta} /></>;
}
