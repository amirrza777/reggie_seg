...
export default async function StaffTeamOverviewPage({ params }: PageProps) {
  ...
  const [requestsResult, meetingsResult, repoHealthResult, peerAssessmentResult] = await Promise.allSettled([
    ...
    getTeamDetails(user.id, project.moduleId, numericTeamId),
  ]);
          ...
          {healthFlags.length === 0 ? (
            ...
            <ul style={{ margin: 0, paddingLeft: 20 }}>
              {healthFlags.map((flag) => (
                <li key={flag} className="staff-projects__team-count">
                  {flag}
                </li>
              ))}
            </ul>
