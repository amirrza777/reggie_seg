...
import ProjectTrelloPage from "./page";
...
vi.mock("@/features/trello/views/TrelloSummaryView", () => ({
  TrelloSummaryView: function TrelloSummaryView() {
    return null;
  },
}));

describe("ProjectTrelloPage", () => {
  beforeEach(() => {
    gateState.teamId = 77;
    gateState.teamName = "Team Orbit";
  ...
  it("passes gate settings and maps null deadline to undefined", async () => {
    const page = await ProjectTrelloPage({ params: Promise.resolve({ projectId: "44" }) });
    render(page);

    expect(screen.getByTestId("gate")).toHaveAttribute("data-project-id", "44");
    ...
    expect(screen.getByTestId("gate")).toHaveAttribute(
      ...
      "Please sign in to view Trello details for this project.",
    );
    expect(screen.getByTestId("trello-content")).toHaveAttribute("data-has-deadline", "false");
    expect(screen.getByTestId("trello-content")).toHaveAttribute("data-view-component", "TrelloSummaryView");
  });

  it("passes through a provided deadline object", async () => {
    gateState.deadline = { assessmentDueDate: "2099-01-01T00:00:00.000Z" };

    const page = await ProjectTrelloPage({ params: Promise.resolve({ projectId: "45" }) });
    render(page);
