...
describe("enterpriseAdmin service.overview-search", () => {
  ...
  it("paginates pinned user search from unpinned slice when page starts after pins", async () => {
    mockState.prisma.user.count.mockResolvedValueOnce(6);
    mockState.prisma.user.findMany
      .mockResolvedValueOnce([
        { id: 501, email: "pin@x.com", firstName: "Pin", lastName: "One", active: true },
      ])
      .mockResolvedValueOnce([
        { id: 601, email: "a@x.com", firstName: "A", lastName: "One", active: true },
        { id: 602, email: "b@x.com", firstName: "B", lastName: "Two", active: true },
      ]);
    mockState.fuzzy.shouldUseFuzzyFallback.mockReturnValueOnce(false);
    const result = await searchAssignableUsers(adminUser as any, {
      scope: "all",
      ...
      page: 2,
      pageSize: 2,
      prioritiseUserIds: [501],
      excludeEnrolledInModuleId: 7,
    } as any);
    expect(result).toEqual(expect.objectContaining({ total: 6, items: [expect.objectContaining({ id: 601 }), expect.objectContaining({ id: 602 })] }));
    expect(mockState.accessSearch.buildEnterpriseAccessUserSearchWhere).toHaveBeenCalledWith(
      "ent-1",
      expect.objectContaining({ scope: "all", query: "a", page: 2, pageSize: 2 }),
      { excludeEnrolledInModuleId: 7, excludeOnModuleParticipation: "all" },
    );
    expect(mockState.prisma.user.findMany).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        skip: 1,
        take: 2,
        where: expect.objectContaining({ AND: expect.arrayContaining([expect.anything(), { id: { notIn: [501] } }]) }),
      }),
    );
  });
  it("paginates entirely within pinned users when page is inside pinned window", async () => {
    mockState.prisma.user.count.mockResolvedValueOnce(4);
    mockState.prisma.user.findMany.mockResolvedValueOnce([
      { id: 11, email: "a@x.com", firstName: "A", lastName: "One", active: true },
      { id: 12, email: "b@x.com", firstName: "B", lastName: "Two", active: true },
      { id: 13, email: "c@x.com", firstName: "C", lastName: "Three", active: true },
    ]);
    mockState.fuzzy.shouldUseFuzzyFallback.mockReturnValueOnce(false);
    const result = await searchAssignableUsers(adminUser as any, {
      scope: "all",
      ...
      page: 1,
      pageSize: 2,
      prioritiseUserIds: [11, 12, 13],
    } as any);
    expect(result).toEqual(expect.objectContaining({ total: 4, items: [expect.objectContaining({ id: 11 }), expect.objectContaining({ id: 12 })] }));
    expect(mockState.prisma.user.findMany).toHaveBeenCalledTimes(1);
  });
  it("merges pinned and unpinned users when page spans both segments", async () => {
    mockState.prisma.user.count.mockResolvedValueOnce(5);
    mockState.prisma.user.findMany
      .mockResolvedValueOnce([
        { id: 21, email: "p1@x.com", firstName: "P1", lastName: "One", active: true },
        { id: 22, email: "p2@x.com", firstName: "P2", lastName: "Two", active: true },
      ])
      .mockResolvedValueOnce([
        { id: 31, email: "u1@x.com", firstName: "U1", lastName: "Three", active: true },
      ]);
    mockState.fuzzy.shouldUseFuzzyFallback.mockReturnValueOnce(false);
    const result = await searchAssignableUsers(adminUser as any, {
      scope: "all",
      ...
      page: 1,
      pageSize: 3,
      prioritiseUserIds: [21, 22],
    } as any);
    expect(result).toEqual(
      expect.objectContaining({
        total: 5,
        items: [expect.objectContaining({ id: 21 }), expect.objectContaining({ id: 22 }), expect.objectContaining({ id: 31 })],
      }),
    );
    expect(mockState.prisma.user.findMany).toHaveBeenNthCalledWith(
      2,
  ...
  it("passes exclude-on-participation options into strict and fuzzy where builders", async () => {
    mockState.prisma.user.count.mockResolvedValueOnce(0).mockResolvedValueOnce(0);
    mockState.prisma.user.findMany.mockResolvedValueOnce([]);
    mockState.fuzzy.shouldUseFuzzyFallback.mockReturnValueOnce(true);
    const result = await searchAssignableUsers(adminUser as any, {
    ...
    expect(result).toEqual(expect.objectContaining({ total: 0, items: [] }));
    expect(mockState.accessSearch.buildEnterpriseAccessUserSearchWhere).toHaveBeenNthCalledWith(
      1,
      "ent-1",
      expect.objectContaining({ scope: "staff", query: "zzz" }),
      { excludeEnrolledInModuleId: 7, excludeOnModuleParticipation: "all" },
    );
    expect(mockState.accessSearch.buildEnterpriseAccessUserSearchWhere).toHaveBeenNthCalledWith(
      2,
