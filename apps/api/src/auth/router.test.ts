...
async function withProductionGoogleEnv(run: () => Promise<void>) {
...
it("registers base auth routes and 503 google routes when oauth is disabled", async () => {
  ...
  expect(routes).toEqual(expect.arrayContaining(BASE_ROUTE_EXPECTATIONS));
...
it("google callback uses secure cookie settings in production", async () => {
  await withProductionGoogleEnv(async () => {
    const res = await invokeGoogleCallback({ id: 5, email: "u@test.com" });
    expect(res.cookie).toHaveBeenCalledWith(
      "refresh_token",
      "r",
      expect.objectContaining({ secure: true, sameSite: "none", domain: "example.com" }),
    );
    expect(res.redirect).toHaveBeenCalledWith(
